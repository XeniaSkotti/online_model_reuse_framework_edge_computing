...
\begin{algorithm}
    ...
    \KwData{$\boldsymbol{samples}$: dictionary associating each node (pi2-pi5) with a sample, $\boldsymbol{models}$: dictionary associating each node with it's OCSVM node model, $\boldsymbol{threshold}$: the minimum probability value for nodes to be considered similar}
