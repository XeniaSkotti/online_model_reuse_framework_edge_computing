...
%experiments we have utilised 
we adopt the linear and Gaussian RBF kernels %, a special case of when data are centered around the origin, 
as defined as:
...
%. Formally, the primary objective is:
...
\end{center}
where $\xi_i$ is the slack variable for a point $i$ which allows it to lie on the other side of the decision boundary, $n$ is the size of the training dataset and $\nu \in (0,1)$ is a regularization parameter. As shown in (\ref{eqn:7}) the objective is not only to minimise the distance of the origin to the decision boundary but also minimise the slack variables $\xi_i$ for all points. %The regularization parameter 
...
\end{center}
where $Q$ is the kernel matrix and $a$ the Lagrange multipliers. Now, %Using the above, 
the decision function becomes:
\begin{equation}
    f(x) = sign(\sum_{i=1}^n a_i k(x, x_i)).
\end{equation}
%For OCSVM we have utilised the RBF kernel as defined by (\ref{eqn:rbf}).
...
% \end{equation}
% where $N_{SV}$ denotes the number of support vectors and $k(x_i,$ $x_p)$ the kernel. In our experiments, we have set the SVR kernel to either be a linear or an RBF kernel (Equations \ref{eqn:linear} and \ref{eqn:rbf} respectively).
...
\begin{algorithm}
    \DontPrintSemicolon
    \caption{Average Similarity MMD (ASMMD) between Nodes' Datasets.
    }\label{alg:asmmd}
...
Algorithm \ref{alg:asmmd} requires that we categorise nodes into two sets, one where all nodes are similar to each other and the rest of them. Categorising nodes in these categories differs when using a regression and classification dataset. We discuss this further in Section \ref{chap:asmmd_parameters}. 
...
\begin{algorithm}
    \DontPrintSemicolon
    \caption{Reusability Maximizing Decision Making Algorithm}
    ...
    \SetKw{KwTo}{in}
         ...
         \For{node \KwTo nodes}{
            ...
            \For{x, y \KwTo $node\_similar\_pairs$}{
                ...
                \tcp{ensures we do not encounter the pair again}
...
\begin{figure}
    \centering
    \includegraphics[scale=0.8]{framework_diagram.jpeg}
